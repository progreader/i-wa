...
#define kMJDismissButton @"kMJDismissButton"
#define kMJDismissDelegate @"kMJDismissDelegate"
#define kMJSourceViewTag 23941
#define kMJPopupViewTag 23942
#define kMJOverlayViewTag 23945

@interface UIViewController (MJPopupViewControllerPrivate) <MJPopupCloseDelegate>
...
- (UIButton *)mj_dismissButton {
    return objc_getAssociatedObject(self, kMJDismissButton);
}

- (void)setMj_dismissButton:(UIButton *)mj_dismissButton {
    objc_setAssociatedObject(self, kMJDismissButton, mj_dismissButton, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (id)mj_dismissDelegate {
    return objc_getAssociatedObject(self, kMJDismissDelegate);
}

- (void)setMj_dismissDelegate:(id)mj_popupViewController {
    objc_setAssociatedObject(self, kMJDismissDelegate, mj_popupViewController, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)presentPopupViewController:(UIViewController *)popupViewController animationType:(MJPopupViewAnimation)animationType isBackgroundClickable:(BOOL)isBackgroundClickable dismissed:(void(^)(void))dismissed
{
    ...
    [self presentPopupView:popupViewController.view animationType:animationType whiteOverlayAlpha:-1 isBackgroundClickable:isBackgroundClickable dismissed:dismissed];
}
...
{
    [self presentPopupViewController:popupViewController animationType:animationType isBackgroundClickable:isBackgroundClickable dismissed:nil];
}
...
{
    self.mj_popupViewController = popupViewController;
    [self presentPopupView:popupViewController.view animationType:animationType whiteOverlayAlpha:whiteOverlayAlpha isBackgroundClickable:isBackgroundClickable dismissed:dismissed];
}
...
{
    [self presentPopupViewController:popupViewController animationType:animationType whiteOverlayAlpha:whiteOverlayAlpha isBackgroundClickable:isBackgroundClickable dismissed:nil];
}
...
- (void)dismissPopupViewControllerWithanimationType:(MJPopupViewAnimation)animationType
{
    ...
    switch (animationType) {
        ...
        case MJPopupViewAnimationSlideRightRight:
            ...
            break;
        case MJPopupViewAnimationZoomInOut:
            [self zoomViewOut:popupView sourceView:sourceView overlayView:overlayView];
            break;
        default:
...
- (void)presentPopupView:(UIView *)popupView animationType:(MJPopupViewAnimation)animationType whiteOverlayAlpha:(float)whiteOverlayAlpha isBackgroundClickable:(BOOL)isBackgroundClickable
{
    [self presentPopupView:popupView animationType:animationType whiteOverlayAlpha:whiteOverlayAlpha isBackgroundClickable:isBackgroundClickable dismissed:nil];
}

- (void)presentPopupView:(UIView *)popupView animationType:(MJPopupViewAnimation)animationType whiteOverlayAlpha:(float)whiteOverlayAlpha isBackgroundClickable:(BOOL)isBackgroundClickable dismissed:(void(^)(void))dismissed
{
    UIView *sourceView = [self topView];
    ...
    if (whiteOverlayAlpha >= 0) {
        ...
        overlayView.alpha = whiteOverlayAlpha;
    } else {
    ...
    self.mj_dismissButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.mj_dismissButton.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    self.mj_dismissButton.backgroundColor = [UIColor clearColor];
    self.mj_dismissButton.frame = sourceView.bounds;
    [self.mj_dismissButton setEnabled:isBackgroundClickable];
    [overlayView addSubview:self.mj_dismissButton];
    ...
    [self.mj_dismissButton addTarget:self action:@selector(dismissPopupViewControllerWithanimation:) forControlEvents:UIControlEventTouchUpInside];
    switch (animationType) {
        case MJPopupViewAnimationSlideBottomTop:
        case MJPopupViewAnimationSlideBottomBottom:
        case MJPopupViewAnimationSlideTopTop:
        ...
        case MJPopupViewAnimationSlideRightLeft:
        case MJPopupViewAnimationSlideRightRight:
            self.mj_dismissButton.tag = animationType;
            [self slideViewIn:popupView sourceView:sourceView overlayView:overlayView withAnimationType:animationType];
            break;
        case MJPopupViewAnimationZoomInOut:
            self.mj_dismissButton.tag = animationType;
            [self zoomViewIn:popupView sourceView:sourceView overlayView:overlayView];
            break;
        default:
            self.mj_dismissButton.tag = MJPopupViewAnimationFade;
...
{
    if (self.mj_dismissDelegate && [self.mj_dismissDelegate respondsToSelector:@selector(dismiss:)]) {
        [self.mj_dismissDelegate dismiss:sender];
    }
}
...
- (void)zoomViewIn:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView
{
    // Generating Start and Stop Positions
    CGSize sourceSize = sourceView.bounds.size;
    CGSize popupSize = popupView.bounds.size;
    ...
    popupView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 0.0f, 0.0f);
    [UIView animateWithDuration:kPopupModalAnimationDuration animations:^{
        [self.mj_popupViewController viewWillAppear:NO];
        self.mj_popupBackgroundView.alpha = 0.5f;
        popupView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 1.0f, 1.0f);
        popupView.alpha = 1.0f;
    } completion:^(BOOL finished) {
        [self.mj_popupViewController viewDidAppear:NO];
    }];
...
- (void)zoomViewOut:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView
{
    [UIView animateWithDuration:kPopupModalAnimationDuration animations:^{
        [self.mj_popupViewController viewWillDisappear:NO];
        self.mj_popupBackgroundView.alpha = 0.0f;
