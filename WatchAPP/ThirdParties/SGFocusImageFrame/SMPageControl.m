...
//	CGContextFillRect(context, CGRectMake(left, 0, size.width, size.height));
	...
	for (NSUInteger i = 0; i < _numberOfPages; i++) {
		...
		if (i == _displayedPage) {
			fillColor = _currentPageIndicatorTintColor ? _currentPageIndicatorTintColor : HEXCOLOR(0xFF820F);
			image = _currentPageImages[@(i)];
			if (nil == image) {
				image = _currentPageIndicatorImage;
			}
		} else {
			fillColor = _pageIndicatorTintColor ? _pageIndicatorTintColor : [HEXCOLOR(0xFF820F) colorWithAlphaComponent:0.3f];
			image = _pageImages[@(i)];
			if (nil == image) {
